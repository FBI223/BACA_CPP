...
bool BMP::SetBitDepth(int NewDepth) {
 ...
 if (BitDepth == 1 || BitDepth == 4 || BitDepth == 8) { Colors = new RGBApixel [NumberOfColors]; } else {
  Colors = NULL;
 }
...
bool BMP::WriteToFile(const char *FileName) {
 ...
 if (XPelsPerMeter) { bmih.biXPelsPerMeter = XPelsPerMeter; } else { bmih.biXPelsPerMeter = DefaultXPelsPerMeter; }
 if (YPelsPerMeter) { bmih.biYPelsPerMeter = YPelsPerMeter; } else { bmih.biYPelsPerMeter = DefaultYPelsPerMeter; }
...
bool CreateGrayscaleColorTable(BMP &InputImage) {
 ...
 ebmpBYTE StepSize;
 if (BitDepth != 1) { StepSize = 255 / (NumberOfColors - 1); } else { StepSize = 255; }
...
bool Rescale(BMP &InputImage, char mode, int NewDimension) {
 ...
 if (CapMode == 'F') {
  if (OldWidth > OldHeight) { CapMode = 'W'; } else { CapMode = 'H'; }
 }
